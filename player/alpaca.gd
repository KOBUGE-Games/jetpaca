...
var ATTACK_SPEED=950
var ATTACK_DURATION=0.7#0.8
var ATTACK_PREPARE_DURATION=0.15
var attack_time=0
var ATTACK_SLIDE_TRESHOLD=50
var ATTACK_TIME_TRESHOLD=0.4
...
#Closest Enemy (for aiming)	
	...
	#if (enemies_to_attack.size()):
	#	print("ETA: ",enemies_to_attack.size())
...
#	print("on floor? ",on_floor)
	...
	if (attack_begin):
		...
		attack_enemy=null
#		if (not on_floor and attack_time==0):
		if (attack_time==0):
			...
			#lv.y=ATTACK_SPEED
			if (attack_vector.dot(Vector2(0,1)) > PI/4.0):
				#Down
				get_node("animation").play("attack_down_1start")
				anim="attack_down_2loop"
			elif (attack_vector.dot(Vector2(0,-1)) > PI/4.0):
				#Up
				...
				if (attack_vector.dot(frontvec) > PI/4.0):
					#Front
					...
					if (not flipping):
						...
						#get_node("animation").play("attack_back_1start")
						#anim="attack_back_2loop"
...
#			v=1.0
	...
	if (attack_time==0 and can_thrust ):
		#print(attack_time==0)	
		#print((!on_floor and (moving_left or moving_right)))
		#print((moving_left and moving_right))
		jetpack_on_fire=true
...
#	if (new_siding_left!=siding_left):
	...
	if (found_floor):
		...
		#print("floorspeed: ",prev_floor_speed)
		lv+=prev_floor_speed
...
func _unhandled_input(event):
	...
	if (event.type==InputEvent.SCREEN_DRAG):
		...
		if ( (slide_1_time <  ATTACK_TIME_TRESHOLD and slide_1.length() > ATTACK_SLIDE_TRESHOLD) or (slide_2_time <  ATTACK_TIME_TRESHOLD and slide_2.length() > ATTACK_SLIDE_TRESHOLD)):
...
#	print(attack_area_offset)
#	get_node("attack_area").set_pos( -attack_area_offset )
...
#	camera_adj_vspeed*=abs(lv.y)*0.001
...
#	get_node("camera").set_zoom(Vector2(3,3))
